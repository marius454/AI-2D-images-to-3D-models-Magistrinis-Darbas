...
class ThreeD_gan(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        tf.cond(disc_accuracy < 0.8, lambda:
            self.d_optimizer.apply_gradients(zip(disc_tape.gradient(disc_loss, self.discriminator.trainable_weights), self.discriminator.trainable_weights)),
            lambda: False)
        ...
        #     self.g_optimizer.apply_gradients(zip(gen_tape.gradient(gen_loss, self.generator.trainable_weights), self.generator.trainable_weights)),
        #     lambda: False)
